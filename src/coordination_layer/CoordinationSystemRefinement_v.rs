...
verus! {
  ...
  // pub proof fn garbage(
  //   v: AbstractJournal::State,
  //   vp: AbstractJournal::State,
  //   label: AbstractJournal::Label
  // )
  // {
  //   assert(AbstractJournal::State::next(v, vp, label));
  ...
  {
    // TODO: remove this assumption once we can reveal AbstractJournal::State::next
    // assume(false);
    ...
    // TODO: prove subset relation in both directions
    assert forall |k| left.value.dom().contains(k) implies right.value.dom().contains(k)
    by
    {
      // What do we know about the relationship between the right and the left that would
      // make this true?
      assert(v.mapadt.persistent.value.contains_key(k) || v.journal.i().contains_key(k));
      // They both come from apply_to_stamped_map, I need to learn facts from there...
      // there will also be some relation that comes from the transition
    }
    assert forall |k| right.value.dom().contains(k) implies left.value.dom().contains(k)
    by
    {

    }

    assert(left.value.dom().ext_equal(right.value.dom()));
    assert_sets_equal!(left.value.dom(), right.value.dom());

    // TODO: Show that mappings from keys to values are equal
    // assert(forall |k| #![auto] left.value.dom().contains(k) ==> left.value[k] == right.value[k]);
